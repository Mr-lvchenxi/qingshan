  ...
  methods: {
    ...
    async handleDrawBackground(context: any, canvas: any) {
      ...
      await new Promise(resolve => {
        image.onload = resolve
        image.src =
          'https://m.xiwang.com/resource/skDsY4UqmXSzlZ0NBcCNj-1727406906687.png'
      })
    ...
    async handleDrawText(ctx: any, canvas: any) {
      ...
      const text3 = '用户userName'
      ctx.font = "bold 30px DINExp";
      ctx.fillStyle = "#fff";
      ctx.fillText(text3, 150, 95); 

      ctx.textAlign="left"

      const textA = 'Content'
      ctx.font = "bold 112px DINExp";
      ctx.fillStyle = "#fff";
      ctx.fillText(textA, 46, 300); 

      ctx.textAlign="left"

      const textB = `英/'kɑntɛnt/`
      ctx.font = "24px PingFangSC";
      ctx.fillStyle = "#fff"
      ctx.fillText(textB, 46, 350); 

      ctx.textAlign="left"

      const textC = `美/'kɑntɛnt/`
      ctx.font = "24px PingFangSC";
      ctx.fillStyle = "#fff"
      ctx.fillText(textC, 46, 400); 

      const textD = `adj.满足的；`
      ctx.font = "24px PingFangSC";
      ctx.fillStyle = "#fff"
      ctx.fillText(textD, 46, 450); 
